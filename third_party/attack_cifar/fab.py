...
class FAB(Attack):
    ...
    Arguments:
        ...
        norm (str) : Lp-norm to minimize. [np.inf, 2, 1] (Default: np.inf)
    ...
    Shape:
        ...
        - output: :math:`(N, C, H, W)`.
    Examples::
        >>> attack = torchattacks.FAB(model, norm=np.inf, steps=100, eps=None, n_restarts=1, alpha_max=0.1, eta=1.05, beta=0.9, loss_fn=None, verbose=False, seed=0, targeted=False, n_classes=10)
        >>> adv_images = attack(images, labels)
    """
    def __init__(self, model, norm=np.inf, eps=None, steps=100, n_restarts=1,
        ...
        Default_EPS_DICT_BY_NORM = {np.inf: .3, 2: 1., 1: 5.0}
