...
class APGD(Attack):
    ...
    Examples::
        >>> attack = torchattacks.APGD(model, norm=np.inf, eps=8/255, steps=100, n_restarts=1, seed=0, loss='ce', eot_iter=1, rho=.75, verbose=False)
        >>> adv_images = attack(images, labels)
    """
    def __init__(self, model,device, norm=np.inf, eps=8/255, steps=100, n_restarts=1, 
...
class APGDT(Attack):
    ...
    Shape:
        ...
        - output: :math:`(N, C, H, W)`.
    Examples::
        >>> attack = torchattacks.APGDT(model, norm=np.inf, eps=8/255, steps=100, n_restarts=1, seed=0, eot_iter=1, rho=.75, verbose=False, n_classes=10)
        >>> adv_images = attack(images, labels)
    """
    def __init__(self, model, device, norm=np.inf, eps=8/255, steps=100, n_restarts=1,
