...
class Square(Attack):
    ...
    Arguments:
        ...
        norm (str): Lp-norm of the attack. [np.inf, 2] (Default: np.inf)
    ...
    Shape:
        ...
        - output: :math:`(N, C, H, W)`.
    Examples::
        >>> attack = torchattacks.Square(model, model, norm=np.inf, n_queries=5000, n_restarts=1, eps=None, p_init=.8, seed=0, verbose=False, targeted=False, loss='margin', resc_schedule=True)
        >>> adv_images = attack(images, labels)
    """
    def __init__(self, model,device,_targeted, norm=np.inf, eps=None, n_queries=5000, n_restarts=1,
    ...
    def attack_single_run(self, x, y):
        with torch.no_grad():
            ...
            if self.norm == np.inf:
                ...
                x_best = torch.clamp(x + self.normalize1(delta_init
                    ) * self.eps, 0., 1.)
                margin_min, loss_min = self.margin_and_loss(x_best, y)
                n_queries = torch.ones(x.shape[0]).to(self.device)
                s_init = int(math.sqrt(self.p_init * n_features / c))
                ...
                for i_iter in range(self.n_queries):
                    ...
                    x_new = torch.clamp(x_curr + self.normalize1(delta_curr
                        ) * self.eps, 0. ,1.)
                    x_new = self.check_shape(x_new)
